...
/**
 * Tax calculation engine that resolves country-specific tax rules
 * and computes the tax amount for a given subtotal.
 * <p>
 * Resolution order (most specific wins):
 * 1. Rules matching country + region (if region specified)
 * 2. Rules matching country only (region is null)
 * <p>
...
public class TaxCalculationService {
    ...
    public TaxCalculationResult calculate(BigDecimal subtotal, String countryCode, String state) {
        ...
        if (state != null && !state.isBlank()) {
            applicableRules = countryRules.stream()
                    .filter(r -> state.equalsIgnoreCase(r.getRegion()))
                    .toList();
            // Fall back to country-level rules if no region match
            if (applicableRules.isEmpty()) {
                applicableRules = countryRules.stream()
                        .filter(r -> r.getRegion() == null || r.getRegion().isBlank())
                        .toList();
            }
        } else {
            applicableRules = countryRules.stream()
        ...
        CountryTax primaryRule = applicableRules.stream()
                .filter(r -> r.getAppliesTo() == null || r.getAppliesTo().contains("General"))
                .findFirst()
        ...
        appliedRates.add(AppliedRate.builder()
                .name(rateName)
                .rate(primaryRule.getRate().doubleValue())
                .amount(taxAmount.doubleValue())
                .build());

        log.debug("Tax calculated: country={}, subtotal={}, taxAmount={}, total={}",
                countryCode, subtotal, taxAmount, total);
