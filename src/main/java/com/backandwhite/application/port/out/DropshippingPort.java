...
public interface DropshippingPort {
    ...
    /**
     * Fetches inventory per variant for a given product pid.
     * Uses /product/stock/getInventoryByPid endpoint.
     */
    List<CjInventoryByPidItemDto> getInventoryByPid(String pid);

    /**
     * Fetches a page of product reviews (comments) from CJ.
     * Uses /product/productComments endpoint.
     *
     * @param pid   product id
     * @param score filter by star rating (0 = all)
     ...
     * All filter parameters are optional — pass null to omit.
     *
     * @param page       1-based page number (max 1000)
     * @param size       page size (max 100)
     * @param categoryId L3 category ID filter
     * @param keyword    search keyword filter
     * @param timeStart  filter products created after this timestamp (millis)
     * @param timeEnd    filter products created before this timestamp (millis)
     * @param orderBy    0=bestMatch, 1=listedCount, 2=sellPrice, 3=createTime,
     *                   4=inventory
     * @param sort       "desc" or "asc"
     */
    CjProductListPageDto getProductListFiltered(int page, int size, String categoryId,
