...
import com.backandwhite.core.kafka.avro.StockReservedEvent;
...
/**
 * Consumes stock events published by mic-orderservice to update
 * inventory in the product catalog database.
 */
@Log4j2
@Service
...
public class StockEventConsumerService {
    ...
    public void onStockDeducted(StockDeductedEvent event) {
        log.info("::> Received stock.deducted: vid={}, orderId={}, qty={}",
                event.getVariantId(), event.getOrderId(), event.getQuantity());
        try {
            inventoryUseCase.deductStock(
                    str(event.getVariantId()),
                    str(event.getOrderId()),
                    event.getQuantity(),
                    null);
        } catch (Exception e) {
            log.error("::> Failed processing stock.deducted for vid={}: {}",
    ...
    public void onStockReleased(StockReleasedEvent event) {
        log.info("::> Received stock.released: vid={}, orderId={}, qty={}",
                event.getVariantId(), event.getOrderId(), event.getQuantity());
        try {
            ...
            log.error("::> Failed processing stock.released for vid={}: {}",
                    event.getVariantId(), e.getMessage(), e);
        }
    }
    ...
    public void onStockRestored(StockRestoredEvent event) {
        log.info("::> Received stock.restored: vid={}, orderId={}, qty={}",
                event.getVariantId(), event.getOrderId(), event.getQuantity());
        try {
            inventoryUseCase.restoreStock(
                    str(event.getVariantId()),
                    str(event.getOrderId()),
                    event.getQuantity(),
                    null);
        } catch (Exception e) {
            log.error("::> Failed processing stock.restored for vid={}: {}",
                    event.getVariantId(), e.getMessage(), e);
        }
    }
