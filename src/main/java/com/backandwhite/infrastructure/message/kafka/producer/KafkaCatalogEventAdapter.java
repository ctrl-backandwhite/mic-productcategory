...
public class KafkaCatalogEventAdapter implements CatalogEventPort {
    ...
    public void publishProductCreated(String productId, String name, String sku, String price, String categoryId,
            String brandId) {
        ProductCreatedEvent event = ProductCreatedEvent.newBuilder().setProductId(productId).setName(name).setSku(sku)
                .setPrice(price != null ? price : "0").setCategoryId(categoryId).setBrandId(brandId).setEnabled(false)
    ...
    public void publishProductUpdated(String productId, String name, String price, String categoryId, String brandId,
            boolean enabled) {
        ProductUpdatedEvent event = ProductUpdatedEvent.newBuilder().setProductId(productId).setName(name)
                .setPrice(price != null ? price : "0").setCategoryId(categoryId).setBrandId(brandId).setEnabled(enabled)
                .setTimestamp(Instant.now().toString()).build();
    ...
    public void publishProductStateChanged(String productId, String name, boolean enabled) {
        ProductStateChangedEvent event = ProductStateChangedEvent.newBuilder().setProductId(productId).setName(name)
                .setEnabled(enabled).setTimestamp(Instant.now().toString()).build();
        String topic = enabled ? AppConstants.KAFKA_TOPIC_PRODUCT_ENABLED : AppConstants.KAFKA_TOPIC_PRODUCT_DISABLED;
    ...
    public void publishCategoryUpdated(String categoryId, String name, String parentId, boolean enabled) {
        CategoryUpdatedEvent event = CategoryUpdatedEvent.newBuilder().setCategoryId(categoryId).setName(name)
                .setParentId(parentId).setEnabled(enabled).setTimestamp(Instant.now().toString()).build();
    ...
    public void publishStockReserved(String productId, String variantId, String orderId, int quantity,
        ...
        StockReservedEvent event = StockReservedEvent.newBuilder().setProductId(productId).setVariantId(variantId)
                .setOrderId(orderId).setQuantity(quantity).setRemainingStock(remainingStock)
                .setTimestamp(Instant.now().toString()).build();
    ...
    public void publishStockReleased(String productId, String variantId, String orderId, int quantity,
            int remainingStock) {
        StockReleasedEvent event = StockReleasedEvent.newBuilder().setProductId(productId).setVariantId(variantId)
                .setOrderId(orderId).setQuantity(quantity).setRemainingStock(remainingStock)
                .setTimestamp(Instant.now().toString()).build();
    ...
    public void publishStockDepleted(String productId, String variantId, String productName) {
        StockDepletedEvent event = StockDepletedEvent.newBuilder().setProductId(productId).setVariantId(variantId)
                .setProductName(productName).setTimestamp(Instant.now().toString()).build();
    ...
    public void publishStockRestored(String productId, String variantId, String orderId, int quantity,
            int remainingStock) {
        StockRestoredEvent event = StockRestoredEvent.newBuilder().setProductId(productId).setVariantId(variantId)
                .setOrderId(orderId).setQuantity(quantity).setRemainingStock(remainingStock)
                .setTimestamp(Instant.now().toString()).build();
    ...
    public void publishStockLowAlert(String productId, String variantId, String productName, int currentStock,
            int threshold) {
        StockLowAlertEvent event = StockLowAlertEvent.newBuilder().setProductId(productId).setVariantId(variantId)
                .setProductName(productName).setCurrentStock(currentStock).setThreshold(threshold)
                .setTimestamp(Instant.now().toString()).build();
    ...
    private void send(String topic, String key, SpecificRecord event) {
        ...
        kafkaTemplate.send(topic, key, event).whenComplete((result, ex) -> {
            if (ex != null) {
                ...
                log.debug("::> Event published to '{}', offset: {}", topic, result.getRecordMetadata().offset());
            }
        });
    }
}
