...
public class ProductSearchRepositoryImpl implements ProductSearchRepositoryCustom {
    ...
    @Override
    public SearchPage<ProductDocument> search(
            String query,
                    ...
                    if (query != null && !query.isBlank()) {
                        bool.must(m -> m.multiMatch(mm -> mm
                                ...
                                .fields("name^3", "name.autocomplete^2",
                                        "categoryName^1.5", "brandName^1.5",
        ...
        if (sortBy != null) {
            switch (sortBy) {
                case "price_asc" ->
                    nq = NativeQuery.builder().withQuery(nq.getQuery()).withPageable(pageable)
                            .withHighlightQuery(buildHighlightQuery())
                            .withSort(s -> s.field(sf -> sf.field("price").order(SortOrder.Asc)))
                            .build();
                case "price_desc" ->
                    nq = NativeQuery.builder().withQuery(nq.getQuery()).withPageable(pageable)
                            .withHighlightQuery(buildHighlightQuery())
                            .withSort(s -> s.field(sf -> sf.field("price").order(SortOrder.Desc)))
                            .build();
                case "newest" ->
                    nq = NativeQuery.builder().withQuery(nq.getQuery()).withPageable(pageable)
                            .withHighlightQuery(buildHighlightQuery())
                            .withSort(s -> s.field(sf -> sf.field("createdAt").order(SortOrder.Desc)))
                            .build();
                default -> {
